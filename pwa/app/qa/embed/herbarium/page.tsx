...
import { useState, useEffect, useCallback, useRef } from 'react'; // ← Añadir useRef  
...
function HerbariumPage() {  
  ...
  const lastVisibleRef = useRef<Timestamp | null>(null); // ← Mover useRef aquí  
  ...
  const loadPhotos = useCallback(async (loadMore = false) => {  
    try {  
      if (loadMore) {  
        ...
        setLoading(true);  
        lastVisibleRef.current = null; // Resetear en carga inicial  
      }  
